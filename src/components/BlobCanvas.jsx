...
void main() {
  ...
  vec2 mouseDist = position.xy - uMouse;
  float dist = length(mouseDist);
  float effect = smoothstep(1.5, 0.0, dist);
  
  vec3 newPos = position + normal * (noise * 0.2) + normal * (effect * 0.5);
...
void main() {
  ...
  vec3 color2 = vec3(0.1, 0.2, 0.35);
  vec3 color3 = vec3(0.0, 0.0, 0.05);
  
  vec3 finalColor = mix(color1, color2, vNoise + 0.5);
  finalColor = mix(finalColor, color3, vUv.y);
